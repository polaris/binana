...
(defun read-segment-load-command-64 (input)
  (let ((result (make-hash-table)))
    (setf (gethash 'cmd result) "SEGMENT_64")
    (setf (gethash 'segname result) (read-string input 8 16))
    (setf (gethash 'vmaddr result) (read-32-bit-word input 16))
    (setf (gethash 'vmsize result) (read-32-bit-word input 20))
    (setf (gethash 'fileoff result) (read-32-bit-word input 24))
    (setf (gethash 'filesize result) (read-32-bit-word input 28))
    (setf (gethash 'maxprot result) (read-32-bit-word input 32))
    (setf (gethash 'initprot result) (read-32-bit-word input 36))
    (setf (gethash 'nsects result) (read-32-bit-word input 40))
    (setf (gethash 'flags result) (read-32-bit-word input 44))
    result))
